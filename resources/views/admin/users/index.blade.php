            ...
            @foreach ($users as $index=> $user)
                <tr class="hover:bg-gray-700">
                    ...
                    <td class="p-3 border-b border-gray-700">
                        <span class="px-2 py-1 rounded-full text-xs font-semibold {{$user->account_verified_at ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'}}">
                            {{$user->account_verified_at ? 'Yes' : 'No'}}
                        </span>
                    </td>
                    <td class="p-3 border-b border-gray-700">{{ $user->roles ? $user->roles->pluck('name')->implode(', ') : 'No Role' }}</td>
                    <td class="p-3 border-b border-gray-700">
                        <button class="bg-blue-600 text-white px-4 py-2 rounded"
                                onclick="openRoleModal({{$user->id}}, {{json_encode($user->roles ? $user->roles->pluck('id') : [])}})">Change Role
                        </button>
                    </td>
                </tr>
            @endforeach
